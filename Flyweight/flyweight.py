...
class HumanFlyweight:
    ...
    def __init__(self, name, code_of_town, address):
        if address not in self.addressesList:
            self.addressesList.append(address)
            self.humans[code_of_town] = name
        else:
            self.humans[code_of_town] += ', ' + name
...
for human in data:
    HumanFlyweight(human[0], human[1], human[2])

print('-'*10)
print(HumanFlyweight.humans)
